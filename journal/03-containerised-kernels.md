...
**podman-hpc build needs `TMPDIR=/tmp` and `XDG_RUNTIME_DIR=/tmp/run-$UID`.** `TMPDIR` alone isn't enough — buildah uses `XDG_RUNTIME_DIR` for its `runRoot` (where it creates rootfs mounts), and on HPC nodes this defaults to a path under `~/.tmp` which has permission issues.
