...
def build_image() -> None:
    ...
    subprocess.run(
        ...
        env={
            **os.environ,
            "TMPDIR": "/tmp",
            "XDG_RUNTIME_DIR": f"/tmp/run-{os.getuid()}",
...
def start_container(
    ...
    *,
    ...
    for spec in extra_mounts or []:
        mount_args += ["-v", spec]
    gpu_args: list[str] = ["--gpu"] if gpu else []
    r = subprocess.run(
        [
            "podman-hpc",
            "run",
            ...
            *mount_args,
            ...
            "-e",
            f"HOME={home}",
            # podman-hpc --gpu mounts host NVIDIA libs at /usr/lib64, but
            # Ubuntu-based images don't have that path in ldconfig, so
            # libcuda.so is invisible and torch.cuda.is_available() is False.
            *(["-e", "LD_LIBRARY_PATH=/usr/lib64"] if gpu else []),
            "--name",
            name,
            "-d",
            IMAGE_NAME,
